...
from seoul_opendata.models.playloads import ChildSchoolCreate, ChildSchoolData, ChildSchoolDelete, ChildSchoolRead, ChildSchoolUpdate, UserCreate, UserData, UserDelete, UserRead, UserUpdate
...
class ChildSchoolRepository(CRUDRepository):
    ...
    def read_all(self) -> dict[str, ChildSchoolData]:
        data: dict[str, ChildSchoolData] = {}
        ...
        for code, entry in query_result.items():
            data[code] = ChildSchool(**entry).dict()
    
        return data
    
    def read(self, payload: ChildSchoolRead) -> ChildSchool:
        return ChildSchool(**cast(ChildSchoolData, self.repo.child(payload["code"]).get()))
